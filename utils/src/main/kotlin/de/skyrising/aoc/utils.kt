...
fun CharSequence.toIntRange(): IntRange {
    val split = indexOf('-')
...
class ByteView(val bytes: ByteArray, var start: Int, var end: Int) : CharSequence {
    ...
    override fun get(index: Int) = bytes[start + index].toInt().toChar()
    override fun subSequence(startIndex: Int, endIndex: Int) = ByteView(bytes, start + startIndex, start + endIndex)
    override fun toString() = String(bytes, start, end - start, Charsets.ISO_8859_1)
    override fun equals(other: Any?): Boolean {
        if (other !is CharSequence || other.length != length) return false
        for (i in indices) {
            if (bytes[i].toInt() != other[i].code) return false
        }
        return true
    }
...
fun Long.getChars(buf: ByteArray, index: Int): Int {
    var i = this
    var index = index
    val negative = (i < 0)
    if (!negative) i = -i
